...
class SignalAggregator:
    ...
    def __init__(self, window_seconds=180, min_samples=60):
        """
        Args:
        ...
        self.last_cleanup = time.time()  # Track last cleanup to prevent memory leak
    ...
    def add_signal(self, action, score, regime=None, strike=None, trade_type=None, direction=None, contract=None):
        ...
        with self.lock:
            ...
            # CRITICAL: Periodic cleanup every 5 minutes to prevent memory leak
            # After 3 hours without cleanup, 10,800 signals accumulate causing UI freeze
            if time.time() - self.last_cleanup > 300:  # 5 minutes
                self._cleanup_old_signals()
                self.last_cleanup = time.time()

    def get_majority_signal(self):
        """
        Get the majority signal from recent history.
        Optimized for moving markets: 
        1. Calculates Majority based solely on ACTION (BUY CALL, BUY PUT, WAIT)
        2. Selects the most recent strike associated with that action.
        Returns: (action, confidence_pct, sample_count, score_avg, recent_strike)
        """
        with self.lock:
            ...
            # Count by ACTION ONLY (to prevent strike-splitting confidence)
            action_counter = Counter(s['action'] for s in recent_signals)
            most_common_action, count = action_counter.most_common(1)[0]
            confidence = (count / total_count) * 100

            # Find the most recent strike for this action
            majority_strike = None
            for s in reversed(recent_signals):
                if s['action'] == most_common_action:
                    majority_strike = s.get('strike')
                    if majority_strike: break

            # Calculate average score for the majority action
            majority_signals = [s for s in recent_signals if s['action'] == most_common_action]
    ...
    def _cleanup_old_signals(self):
        """Remove signals older than 2x window to prevent memory buildup over long sessions"""
        with self.lock:
            cutoff = time.time() - (self.window_seconds * 2)  # 6 minutes for 3min window
            old_count = len(self.signal_history)
            # Filter out stale signals
            self.signal_history = deque(
                (s for s in self.signal_history if s['timestamp'] >= cutoff),
                maxlen=2000
            )
            new_count = len(self.signal_history)
            if old_count > new_count:
                logger.debug(f"[SIGNAL AGG] Cleaned up {old_count - new_count} old signals ({new_count} remaining)")
...
class TradeLoggerv10:
    """
    Enhanced Trade Logger v10 for Safe Guardian with Signal Aggregation.
    """

    def __init__(self, filename="logs/v10_paper_trades.csv", entry_window=180, exit_window=120, min_samples=15):
        # Date-wise folder: logs/DD-MM-YYYY/v10_{strategy}_trades.csv
        self._base_name = os.path.basename(filename)
        self._log_dir = "logs"
        ...
        self.entry_window_seconds = entry_window
        self.exit_window_seconds = exit_window
        self.entry_min_samples = min_samples
        self.exit_min_samples = max(5, min_samples // 2)
        ...
        # Performance summary cache (prevent CSV re-read every 1s)
        self._perf_cache = None
        self._perf_cache_time = 0
        self._perf_cache_ttl = 30       # Cache for 30 seconds
        ...
        self.headers = [
            ...
            'Max_Profit_Seen',
            'Max_Loss_Seen',
            # === MARKET CONTEXT SNAPSHOT (15 new columns) ===
            'Sig_Delta_OI',      # Individual signal scores at entry/exit
            'Sig_IV_Skew',
            'Sig_OI_Unwind',
            'Sig_Max_Pain',
            'Sig_GEX',
    ...
    def _get_dated_filename(self):
        """Get filename with today's date folder: logs/DD-MM-YYYY/filename.csv"""
        today = datetime.now().strftime("%d-%m-%Y")
        self._current_date = today
        return os.path.join(self._log_dir, today, self._base_name)

    def _check_date_rollover(self):
        """If date has changed, switch to new file in new date folder."""
        today = datetime.now().strftime("%d-%m-%Y")
        if today != self._current_date:
            self.filename = self._get_dated_filename()
            self._initialize_file()
            logger.info(f"[LOGGER] Date rollover: new file {self.filename}")
    ...
    def log_trade(self, trade_data):
        """Log a trade event to CSV (auto-switches to new date folder at midnight)"""
        try:
            ...
            # Invalidate performance cache on new trade
            self._perf_cache = None
    ...
    def _log_signal_status(self, current_action, current_score, regime, underlying_ltp, symbol, current_strike=None, current_contract=None):
        ...
        # Only log at specified interval (unless it's time-critical or FIRST RUN)
        time_since_last_log = current_time - self._last_log_time
        if self._last_log_time > 0 and time_since_last_log < self.log_interval:
            return
        ...
        print(f"\n{'='*75}")
        print(f"{status_icon} [V10] [{timestamp}] {symbol} @ {underlying_ltp:.2f} | {strike_info} | Signals: {self._signal_count}")
        print(f"{'='*75}")
    ...
    def _log_entry_event(self, trade, confidence, distribution):
        ...
        print(f"\n{'*'*70}")
        print(f"{'*'*70}")
        print(f"   🚀 [V10] TRADE ENTRY EXECUTED!")
        print(f"{'*'*70}")
    ...
    def _log_exit_event(self, trade, exit_reason, exit_price, pnl_amount, roi_pct, hold_duration):
        ...
        print(f"\n{'#'*70}")
        print(f"{'#'*70}")
        print(f"   {pnl_icon} [V10] TRADE EXIT - {exit_reason}")
        print(f"{'#'*70}")
        print(f"   📝 Trade ID:   {trade['Trade_ID']}")
        print(f"   📄 Contract:   {trade['Contract']}")
    ...
    def get_recent_trades(self, limit=10):
        ...
        Uses tail-read approach: reads file from end to avoid loading
        entire CSV into memory (which caused UI freezes as CSV grew).
        """
        trades = []
        try:
            if not os.path.exists(self.filename):
            ...
            # Read headers first, then tail lines
            with open(self.filename, 'r') as f:
                ...
                # Use deque with maxlen to keep only last N rows in memory
                from collections import deque
                tail = deque(reader, maxlen=limit)

            # Convert to dicts
            for row in reversed(list(tail)):
    ...
    def get_performance_summary(self):
        """Calculate and return performance metrics (cached to prevent CSV reads every tick)"""
        now = time.time()
