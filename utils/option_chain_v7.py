"""
Option Chain Manager Module v7
...
class OptionChainManagerV7:
    ...
    def __init__(self, underlying, expiry, websocket_manager=None):
        """
        Initialize OptionChainManagerV7
        ...
        self.expiry = expiry
        # print(f"Underlying: {underlying}, Expiry: {expiry}")
        if underlying == 'NIFTY':
            self.strike_step = 50
        ...
        self.manager_id = f"{underlying}_{expiry}_V7"
        ...
        self.pcr_history = deque(maxlen=600) # Track PCR for divergence
        self.oi_history = deque(maxlen=600) # Increased for deeper analysis
        self.vol_history = deque(maxlen=600) # Track total volume flow
        ...
        self.strategy_mode = 'ADAPTIVE' # Default to ADAPTIVE mode

        # ADAPTIVE mode additions
        self.current_regime = 'DETECTING'
        self.regime_confidence = 0
        self.available_modes = ['ADAPTIVE', 'MOMENTUM_RIDER', 'THETA_HUNTER', 'CONTRARIAN_REVERSAL', 'INSTITUTIONAL_SHADOW']

        logger.info(f"Initialized OptionChainManagerV7 for {underlying} (Quote Mode Only)")

    def set_strategy_mode(self, mode):
        """Update strategy mode dynamically"""
        valid_modes = self.available_modes if hasattr(self, 'available_modes') else ['SNIPER', 'SCALPER', 'ADAPTIVE']
    ...
    def _score_momentum_velocity(self):
        """
        Signal 8: Momentum Velocity (A.2)
        Detects rapid price spikes (Velocity > Threshold).
        Returns: (score, reason)
        """
        if len(self.price_history) < 10:
            return 0, "No Velocity Data"
            
        # Check last 10 seconds (assuming ~1 tick/sec or effectively last 10 ticks)
        p_now = self.price_history[-1]
        p_prev = self.price_history[-10]
        
        velocity_pct = ((p_now - p_prev) / p_prev) * 100
        
        # Threshold: 0.1% move in 10 ticks is significant for an index
        if velocity_pct > 0.05: # mild pump
             if velocity_pct > 0.15: return 25, f"🚀 MOMENTUM BURST (+{velocity_pct:.2f}%)"
             return 10, f"Positive Momentum (+{velocity_pct:.2f}%)"
        
        elif velocity_pct < -0.05: # mild dump
             if velocity_pct < -0.15: return -25, f"🔻 MOMENTUM CRASH ({velocity_pct:.2f}%)"
             return -10, f"Negative Momentum ({velocity_pct:.2f}%)"
             
        return 0, "Stable Velocity"
    ...
    def _score_pcr_roc(self):
        """
        Signal 10: PCR Rate-of-Change (Momentum in PCR)
        Detects rapid changes in Put-Call Ratio - more useful than static PCR.
        Returns: (score, reason)
        """
        if len(self.pcr_history) < 20:
            return 0, f"Building PCR History ({len(self.pcr_history)}/20)"
        
        pcr_now = self.pcr_history[-1]
        pcr_prev = self.pcr_history[-20]  # ~2 min ago at 1 tick/sec
        ...
        # PCR surging means Put OI increasing faster than Call OI = Support building
        # Lowered thresholds for normal market sensitivity
        if roc > 5:
            return 20, f"🟢 PCR Surge +{roc:.2f}% (Strong Support Building)"
        elif roc > 2:
            return 10, f"PCR Rising +{roc:.2f}% (Bullish Support)"
        
        # PCR dropping means Call OI increasing faster = Resistance building
        elif roc < -5:
            return -20, f"🔴 PCR Crash {roc:.2f}% (Strong Resistance)"
        elif roc < -2:
            return -10, f"PCR Falling {roc:.2f}% (Bearish Pressure)"
        
        # Show actual ROC even in stable state for transparency
        return 0, f"Stable PCR (ROC: {roc:+.2f}%)"

    def _verify_volume_liquidity(self):
        """
        Signal 11: Volume Confirmation Filter
        Rejects signals when ATM options have insufficient liquidity.
        ...
        # Indices need higher volume, stocks can be lower
    ...
    def _check_advanced_divergence(self):
        ...
        1. PCR Divergence (Price Up, PCR Down = Weakness)
        2. Volume-OI Divergence (High Vol, Flat OI = Churn)
        3. IV Exhaustion (Price Down, IV Down = Bottoming)
        """
        ...
        # 3. IV Exhaustion (The Reversal Signal)
    ...
    def _detect_institutional_flow(self):
        ...
        for strike, data in self.option_data.items():
            ...
            if ce_oi > 0 and ce_vol > ce_oi * 0.8:
                ...
                data['ce_data']['institutional_flow'] = True # Mark for UI
            ...
            if pe_oi > 0 and pe_vol > pe_oi * 0.8:
                ...
                data['pe_data']['institutional_flow'] = True # Mark for UI
                if strike < self.underlying_ltp: score -= 5
            else:
                 data['pe_data']['institutional_flow'] = False
                
        if alerts:
            reason = ", ".join(alerts[:2])
    ...
    def _scan_iv_surface(self):
        ...
        Baseline: ATM IV.
        Cheap: < 85% of ATM IV
        Expensive: > 135% of ATM IV (High Skew)
        """
        if not self.atm_strike or self.atm_strike not in self.option_data:
            return
    ...
    def _calculate_heatmap_intensity(self):
        ...
        Normalizes OI change against the maximum change in the chain.
        """
        ...
        for strike, data in self.option_data.items():
            if strike not in self.initial_state: continue
        ...
        for strike, data in self.option_data.items():
            ...
            # Sign determines sentiment (Addition = Intensity, Unwinding = Different color logic in UI)
    ...
    #     """
    #     New v7 Battle-tested Quant Decision Engine
    ...
    #     # NEW: PCR Rate-of-Change (momentum in PCR)
    ...
    #     base_score = score_delta + score_iv + score_unwind + score_mp + score_gex + score_vc + score_flow + score_mom + score_pcr_roc
        
    #     # Apply expiry multiplier (reduces score near expiry)
    ...
    #     # Calculate OI Change (B.3)
    #     if self.initial_state:
    #          for strike, data in self.option_data.items():
    #              if strike in self.initial_state:
    #                  # Calculate and store OI Change
    ...
    #     # 2. Pattern Discovery Scan (Non-scoring, for UI findings)
    ...
    #         f"PCR Momentum: {score_pcr_roc} ({s_pcr_reason})",
    #     ]
        
    #     # NEW: Add expiry warning if near expiry
    #     if expiry_mult < 1.0:
    #         reasons.insert(0, f"⏰ {expiry_reason} (Score x{expiry_mult})")
        
    #     # NEW: Add liquidity warning if low volume
    #     if not is_liquid:
    ...
    #     # -- ADVANCED DIVERGENCE 2.0 CHECK --
    #     adv_div_type, adv_div_msg = self._check_advanced_divergence()
    #     if adv_div_type:
    #         reasons.insert(0, f"🔍 OPPORTUNITY: {adv_div_msg}")
    #         # Optional: Influence score slightly for "Finding" visibility
    #         if "Hidden Strength" in adv_div_msg or "IV EXHAUSTION" in adv_div_msg:
    #             total_score += 10
    #         elif "Hidden Weakness" in adv_div_msg or "IV SPIKE" in adv_div_msg:
    #             total_score -= 10
    ...
    #     # Calculate PCR Bias independently
    #     pcr_bias = "NEUTRAL"
    ...
    def construct_option_symbol(self, strike, option_type):
        ...
        if isinstance(self.expiry, str):
            try:
                ...
                if len(parts) >= 3:
                    ...
                    expiry_year = parts[2][-2:]  # Get last 2 chars for year
                elif len(parts) >= 2:
                    day = parts[0].zfill(2)
                    month = parts[1].upper()[:3]
                    expiry_formatted = f"{day}{month}"
            ...
            expiry_year = self.expiry.strftime('%y')  # Get 2-digit year
        else:
            expiry_formatted = '28AUG'
        ...
        # Construct symbol: BASE + EXPIRY + YEAR + STRIKE + CE/PE
    ...
    def get_option_chain(self):
        ...
        # Self-healing: If ATM is 0 (Initialization failed), retry
        if self.atm_strike == 0:
            logger.warning(f"[{self.underlying}] ATM is 0, attempting late recovery...")
            if self.calculate_atm() > 0:
                logger.info(f"[{self.underlying}] Late recovery successful. ATM: {self.atm_strike}")
                self.generate_strikes()
                self.setup_subscriptions()
            else:
                logger.warning(f"[{self.underlying}] Late recovery failed. Retrying next tick.")
    ...
    def generate_signals(self, pcr, max_pain):
        ...
        # Aggregate base score with new PCR ROC
        base_score = score_delta + score_iv + score_unwind + score_mp + score_gex + score_vc + score_flow + score_pcr_roc
        
        # Apply expiry multiplier (reduces score near expiry)
        total_score = int(base_score * expiry_mult)
        
        scores_list = [score_delta, score_iv, score_unwind, score_mp, score_gex, score_vc, score_flow, score_pcr_roc]
        ...
        volatility = self._get_recent_volatility() if hasattr(self, '_get_recent_volatility') else 0.1

        # Build scores dict for regime detection
        scores_dict = {
        ...
        # If ADAPTIVE mode is selected, use regime detection
        regime_info = {}
        if self.strategy_mode == 'ADAPTIVE':
            ...
            reasons.insert(0, f"📊 Regime: {regime_info['regime']} (Confidence: {regime_info['confidence']}%)")
            reasons.insert(1, f"Reason: {regime_info['reason']}")

            # Update trade setup if action changed
            if action != 'WAIT':
                bias_for_strike = 'BULLISH' if 'CALL' in action else 'BEARISH' if 'PUT' in action else 'NEUTRAL'
                best_contract = self._select_best_strike(bias_for_strike, action)
                if best_contract:
                    levels = self._calculate_trade_levels(best_contract['ltp'], action)
                    trade_setup = {
                        'contract': best_contract['name'],
            ...
            self.current_regime = self.strategy_mode
            self.regime_confidence = abs(total_score)
