...
class TradeLoggerV9:
    """
    Enhanced Trade Logger V9 for Safe Guardian with Signal Aggregation.
    """

    def __init__(self, filename="logs/v9_paper_trades.csv"):
    ...
    def _log_signal_status(self, current_action, current_score, regime, underlying_ltp, symbol, current_strike=None, current_contract=None):
        ...
        print(f"\n{'='*75}")
        print(f"{status_icon} [V9] [{timestamp}] {symbol} @ {underlying_ltp:.2f} | {strike_info} | Signals: {self._signal_count}")
        print(f"{'='*75}")
    ...
    def _log_entry_event(self, trade, confidence, distribution):
        ...
        print(f"\n{'*'*70}")
        print(f"{'*'*70}")
        print(f"   🚀 [V9] TRADE ENTRY EXECUTED!")
        print(f"{'*'*70}")
    ...
    def _log_exit_event(self, trade, exit_reason, exit_price, pnl_amount, roi_pct, hold_duration):
        ...
        print(f"\n{'#'*70}")
        print(f"{'#'*70}")
        print(f"   {pnl_icon} [V9] TRADE EXIT - {exit_reason}")
        print(f"{'#'*70}")
        print(f"   📝 Trade ID:   {trade['Trade_ID']}")
        print(f"   📄 Contract:   {trade['Contract']}")
