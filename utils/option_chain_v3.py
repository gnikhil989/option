"""
Option Chain Manager Module V3
...
class OptionChainManagerV3:
    ...
    def __init__(self, underlying, expiry, websocket_manager=None):
        """
        Initialize OptionChainManagerV3
        ...
        self.cache = OptionChainCacheV3()
        self.monitoring_active = False
        self.initialized = False
        self.manager_id = f"{underlying}_{expiry}_v3"
        self.option_mode = 'quote'  # Forced Quote Mode
        logger.info(f"Initialized OptionChainManagerV3 for {underlying} (Quote Mode Only)")
    ...
    def generate_strikes(self):
        ...
        for strike_info in strikes:
            ...
            if self.option_data[strike]['tag'] == 'ATM':
                print(self.option_data[strike]['ce_symbol'])
                print(self.option_data[strike]['pe_symbol'])
                ceGreek = self.api_client.optiongreeks(symbol=self.option_data[strike]['ce_symbol'], exchange='NFO')
                peGreek = self.api_client.optiongreeks(symbol=self.option_data[strike]['pe_symbol'], exchange='NFO')
                print(ceGreek)
                print(peGreek)
